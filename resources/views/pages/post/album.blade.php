...
@section('title', 'Album ' . $album->nama_album . ' | Gallery')
@section('section')
<div class="container pt-3 pb-4">
    <div class="row d-flex g-0 w-100 shadow rounded-4 position-relative" style="min-height: 500px;">
                ...
                <div class="swiper-wrapper swiper-wrapper-foto">
                    ...
                    <div class="swiper-slide rounded position-relative bg-light">
                        ...
                        <div class="d-flex gap-2 position-absolute me-4 mb-4" style="bottom: 0; right: 0">
                            <!-- <a download="{{ $foto->judul_foto }}" href="{{ url('storage/upload/' . $foto->lokasi_file) }}" class="text-decoration-none btn bg-white rounded p-0 d-flex align-items-center justify-content-center" style="height: 40px; width: 40px;"><i class="fa-solid fa-download"></i></a> -->
        ...
        <div class="col-12 col-lg-4 h-100 overflow-y-auto position-relative pt-lg-3" style="min-height: 500px">
            <div class="p-3 pb-0">
                <a href="{{ url($album->user->username) }}" class="mb-3 d-flex gap-2 text-decoration-none text-dark">
                    <img src="{{ asset('storage/profil/' . $album->user->profil) }}" class="rounded-circle object-fit-cover" height="50" width="50">
                    <div class="d-flex flex-column justify-content-center">
                        <p class="m-0 wrap-text fw-semibold fs-14px">{{ $album->user->nama_lengkap }}</p>
                        <p class="m-0 wrap-text fw-semibold text-secondary fs-13px">{{ $album->user->username }}</p>
                    </div>
                </a>
                <span>Album</span>
                <div class="d-flex align-items-center justify-content-start gap-2 text-secondary fs-14px text-nowrap">
                    <p class="m-0 fw-semibold"><i class="fa-solid fa-comment"></i> {{ convertNumberToShortFormat($album->foto->sum('komentar_count')) }} Komentar</p>
                    <p class="m-0 fw-semibold"><i class="fa-solid fa-image"></i> {{ convertNumberToShortFormat($album->foto->count()) }} Foto</p>
                    <p class="m-0 fw-semibold"><i class="fa-solid fa-heart"></i> {{ convertNumberToShortFormat($album->foto->sum('like_count')) }} Suka</p>
                </div>
                <h3 class="fw-semibold wrap-text" style="-webkit-line-clamp:3">{{ $album->nama_album }}</h3>
                <p class="fw-semibold fs-14px text-secondary overflow-y-auto" style="max-height: 150px">{{ $album->deskripsi }}</p>
            </div>

            <div class="detail-foto-wrapper">
                @foreach ($album->foto as $foto)
                <div class="detail-foto {{ $loop->first ? '' : 'd-none' }}" id="{{ $foto->id . '-' . $foto->lokasi_file }}">
                    <div class="px-3">
                        <span>Foto</span>
                        <p class="fw-semibold fs-15px judul-foto mb-3 wrap-text" style="-webkit-line-clamp:2">{{ $foto->judul_foto }}</p>
                        <form action="{{ url('foto/like/' . $foto->id) }}" class="border-top border-bottom mb-3 d-flex align-items-center" method="post">
                            @csrf
                            <input type="hidden" name="swiper_index" value="{{ $loop->index }}">
                            ...
                            <button class="text-start flex-grow-1 p-3 h-100 btn rounded-0 border-0 m-0 fw-semibold d-flex align-items-center gap-2 justify-content-center align-items-lg-center btn-submit">
                        ...
                        {{-- @dd($foto) --}}
                        <div class="overflow-y-auto p-2" style="min-height: 150px; max-height: 500px">
                            ...
                            <div class="mb-3 d-flex gap-2">
                                ...
                                <div class="d-flex flex-column justify-content-center bg-light rounded p-2">
                                    ...
                                    <p class="m-0 mb-1 fs-13px">{{ $komentar->isi_komentar }}</p>
                    ...
                    <form action="{{ url('foto/komentar/' . $foto->id) }}" class="w-100 gap-2 p-3 border-top" method="POST">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button id="btn_submit_komentar" disabled class="btn btn-dark rounded-circle p-0 d-flex justify-content-center align-items-center btn-submit" style="height: 40px; width: 40px;">
...
<script>
    ...
    komentar.addEventListener('input', () => {
        const btn = document.getElementById('btn_submit_komentar');
        if (komentar.value) {
    ...
    @if(Session::has('swiper_index'))
    swiper.slideTo({
        {
            Session::get('swiper_index')
        }
    });
    @endif
